...
// Recover video buffer from disk if bot restarted (video was saved in previous session)
try { if (fs.existsSync("/tmp/input_video.mp4")) { lastVideoBuffer = fs.readFileSync("/tmp/input_video.mp4"); console.log(`[startup] recovered lastVideoBuffer from disk (${lastVideoBuffer.length} bytes)`); } } catch {}
try { if (!lastVideoBuffer && fs.existsSync("/tmp/last_generated_video.mp4")) { lastVideoBuffer = fs.readFileSync("/tmp/last_generated_video.mp4"); console.log(`[startup] recovered lastVideoBuffer from last_generated_video (${lastVideoBuffer.length} bytes)`); } } catch {}
...
async function postToBuffer({ text, mediaBuffer, mimeType, channels = ["instagram"] }) {
  ...
  if (mediaBuffer) {
    try {
      ...
      // Videos: confirm=1 bypasses Google's virus-scan interstitial for direct download
      // Images: lh3 thumbnail CDN serves directly without redirect
      mediaUrl = isVideo
        ? `https://drive.google.com/uc?export=download&confirm=1&id=${fileId}`
    ...
    if (interaction.isButton()) {
      ...
      if (action === "video" && (ctx.imageBuf || lastGeneratedImageBuffer)) {
        ...
        try {
          ...
          lastVideoBuffer = stitched; lastVideoMime = "video/mp4";
...
🎵 Music: **ACE-Step** (ComfyUI)`,
    ...
    if (cmd === "video") {
      ...
      try {
        ...
        lastVideoBuffer = videoBuf; lastVideoMime = "video/mp4"; try { fs.writeFileSync("/tmp/last_generated_video.mp4", videoBuf); } catch {} backupMedia(videoBuf, `vid-${Date.now()}.mp4`, "video/mp4"); lastGeneratedImageBuffer = null;
  ...
  try {
    ...
    if (videoMatch) {
      ...
      if (earlyImagePaths.length > 0 && !lastGeneratedImageBuffer) {
        ...
        if (earlyLocal) {
          lastGeneratedImageBuffer = fs.readFileSync(earlyLocal);
      ...
      if (segments.length < 2) {
        ...
        try {
          ...
          if (results.length > 0) {
            ...
            lastVideoBuffer = lastResult.videoBuf;
            lastVideoMime = "video/mp4";
            lastGeneratedImageBuffer = null;
          }
    ...
    if (hasMakeGif) {
      for (const att of msg.attachments.values()) {
        if (att.contentType?.startsWith("video/") || /\.(mp4|mov|webm|avi)$/i.test(att.name || "")) {
          try {
            lastVideoBuffer = await fetchBuffer(att.url);
            lastVideoMime = att.contentType || "video/mp4";
    ...
    if (hasMakeGif && !lastVideoBuffer) {
      for (const f of ["/tmp/input_video.mp4", "/tmp/last_generated_video.mp4"]) {
        try { if (fs.existsSync(f)) { lastVideoBuffer = fs.readFileSync(f); console.log(`[gif] recovered ${lastVideoBuffer.length} bytes from ${f}`); break; } } catch {}
      }
    }
    if (hasMakeGif && lastVideoBuffer) {
      try {
